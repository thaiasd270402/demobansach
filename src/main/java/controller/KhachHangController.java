...
public class KhachHangController extends HttpServlet {
			...
			if (baoLoi.length() > 0) {
				...
				if (khachHangDAO.insert(kh) > 0) {
					...
					if (khachHangDAO.updateVerifyInformation(kh) > 0) {
						...
						Email.sendEmail(kh.getEmail(), "Xác thực tài khoản tại PNT.vn", getNoiDung(kh));
					}
				}
	...
	public static String getNoiDung(KhachHang kh) {
		String link = "http://localhost:8080/demo9_bansach/khach-hang?hanhDong=xac-thuc&maKhachHang="
				+ kh.getMaKhachHang() + "&maXacThuc=" + kh.getMaXacThuc();
		String noiDung = "<p>PNT.vn xin ch&agrave;o bạn <strong>" + kh.getHoVaTen() + "</strong>,</p>\r\n"
